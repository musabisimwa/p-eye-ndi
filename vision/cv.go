...
func NewConfiguration(camera int,title string) (*Config,error){
    captureDevice,captErr := gocv.OpenVideoCapture(camera)
    if captErr !=nil{
        log.Fatalf("configuration initialization err : %v",captErr)
        return nil,nil
    }
    matrix:= gocv.NewMat()
    log.Println("configuration created")
    return &Config{
        video: captureDevice,
...
//crop
func (config *Config) Zoom (scale float32){

    if scale > 1.5 {
        log.Fatalln(" can not scale more than 1.5x")
        return
    }
